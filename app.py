...
# --- 2. КОНФИГУРАЦИЯ API (ОПТИМИЗИРОВАННАЯ) ---
api_key = st.secrets.get("GOOGLE_API_KEY")
# Используем 1.5 Flash для стабильности на Free Tier
...
def get_context(query, chunks):
    ...
    # СОКРАЩЕНО: берем 4 чанка вместо 8 для экономии квоты
    top_chunks = scored[:4] 
...
if prompt := st.chat_input("Запросить данные..."):
    ...
    with st.chat_message("assistant"):
        ...
        with st.spinner("Анализ (режим экономии квоты)..."):
            system_instruction = "Ты инженерный эксперт. Отвечай только по контексту. В конце сделай список '### Ссылки на документацию' с новой строки через дефис по шаблону: - Название, Раздел, стр. №, файл (SOURCE_файл_PAGE_номер). Между ответом и ссылками 2 отступа."
            ...
            try:
                ...
                if 'error' in res_json:
                    clean_answer = f"❌ Ошибка API: {res_json['error'].get('message')}. Попробуйте через минуту."
                    verified_sources = []
        ...
        copy_to_clipboard(clean_answer, "n")
