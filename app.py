...
def get_context(query, chunks):
    ...
    # Список приоритетных ключевых слов в названии файла
    ...
    for c in chunks:
        ...
        # 1. Базовый скоринг по тексту
        if query_low in content_low:
            score += 100 # Точное совпадение фразы
            
        for w in query_words:
            if w in content_low:
                score += 10 # Совпадение ключевых слов
        
        # 2. ПРИМЕНЕНИЕ ВЕСОВ ДОКУМЕНТОВ
        is_priority = any(k in filename_low for k in priority_keywords)
        
        if is_priority:
            score *= 2.0  # Удваиваем вес для приоритетных руководств
        else:
            score *= 0.8  # Слегка снижаем приоритет для остальных (refguide и т.д.)
    ...
    with st.chat_message("assistant"):
        ...
        with st.spinner("Анализирую документацию..."):
            system_instruction = (
                ...
                "Твоя цель: внимательно изучить предоставленные фрагменты текста и найти в них ответ. "
                "Если информация найдена, дай подробный пошаговый алгоритм. "
            ...
            try:
                ...
                if response.text:
                    ...
                    final_answer = "К сожалению, не удалось найти ответ."
                    verified_sources = []
            except Exception as e:
