...
st.caption("⚠️ Ответ сгенерирован ИИ. Сверяйте данные с документацией.")
...
if prompt := st.chat_input(f"Задать вопрос по MaxPatrol SIEM {selected_ver}..."):
    ...
    with st.chat_message("assistant"):
        ...
        with st.spinner("Анализирую документы..."):
            # --- ПРАВКА: СТРОГИЙ ПРОМПТ И ТЕМПЕРАТУРА 0 ---
            system_instruction = """
            Ты — промышленный ИИ-эксперт по системе MaxPatrol SIEM.
            Твоя задача — найти ответ на ВОПРОС пользователя, используя ТОЛЬКО предоставленный КОНТЕКСТ.
            
            ПРАВИЛА:
            1. Ищи ответ буквально, по ключевым словам из вопроса.
            2. Если ответ находится в контексте, приведи его со ссылкой на источник в формате (SOURCE_имя_PAGE_номер).
            3. Если в контексте нет прямого ответа на вопрос, НЕ ВЫДУМЫВАЙ. Напиши: "В предоставленной документации не найдено информации о [тема вопроса]".
            4. Не используй общие знания, только текст контекста.
            """
            ...
            payload = {
                ...
                "temperature": 0.0 # ПРАВКА: Температура 0
            }
